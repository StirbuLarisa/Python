...
class Board :
    ...
    def validate_move(self, current_column_index, new_column_index, player, dice_values, distance):
        if self.current_column_is_empty(current_column_index) and current_column_index != 0 and current_column_index != 25:
            return False
    ...
    def draw_removed_pieces(self,screen,player):
        ...
        for piece in player.removed_pieces:
            ...
            if player.color == "black":
                screen.blit(img, ( (self.x +6*self.triangle_width +1) + self.middle_line_width/2 - img.get_width()/2 , screen.get_height()/2-piece.bounds.height-index*img.get_height()))
            else:
                screen.blit(img, ( (self.x +6*self.triangle_width +1) + self.middle_line_width/2 - img.get_width()/2 ,screen.get_height()/2+index*img.get_height()))
            index += 1
    ...
    def compute_possible_moves(self, player, dice_values, current_column_index = None):
        ...
        if len(player.removed_pieces) == 0:
            for index in range(1,25):
                if self.validate_move(current_column_index, index, player, dice_values, abs(current_column_index - index)):
